...
class User extends BaseModel {
    ...
    public function register($userData) {
        try {
            ...
            // Debug output
            echo "<p>Executing query: " . str_replace([':nom', ':email'], 
                                                     [$name, $userData['email']], 
                                                     $query) . "</p>";
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->execute()) {
                    $this->conn->commit();
                    echo "<p style='color:green'>Password stored in extrafields. Registration complete for user ID: {$userId}</p>";
                    return $userId;
                } else {
                    $this->conn->rollBack();
                ...
                $this->conn->rollBack();
                $error = $stmt->errorInfo();
                echo "<p style='color:red'>Failed to insert user into main table. SQL error: " . json_encode($error) . "</p>";
                return false;
            }
        } catch (Exception $e) {
